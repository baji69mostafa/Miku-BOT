...
/*------------------------------------------------*/
    ...
    if (!('ايزن' in chat)) chat.زورو = false 
...
if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
  ...
  user.commandCount = 1
        ...
        if (!m.fromMem && m.text.match(/(ميكو|بوتي|بوت|ايانو|شرح|Miku|فيديو|اغنيه|اوامر)/gi)) {
...
export async function callUpdate(callUpdate) {
    ...
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
     let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Yosef Ragab 👑;;;\nFN:Yosef Ragab 👑\nORG:Yosef Ragab 👑\nTITLE:\nitem1.TEL;waid=201129498962:+201129498962 \nitem1.X-ABLabel:Yosef Ragab👑\nX-WA-BIZ-◤𝐀𝐈𝐙𝐄𝐍  - 𝐁𝐎𝐓 ◢:[❗] كلم الرقم ده - المطور.\nX-WA-BIZ-NAME:Yosef Ragab 👑\nEND:VCARD`
...
global.dfail = (type, m, conn) => {
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] تنبيه - إشعار', body: 'Miku Bot', thumbnail: imagen1, sourceUrl: 'https://atom.bio/ayanakoji' }}}}, aa)
