const handler = async (m, { conn, command, text }) => {
  ...
  const loveDescription = isHighLove ? "💕" : "نسبه حبكم هي";
  const getRandomMessage = (messages) => messages[Math.floor(Math.random() * messages.length)];
  const loveMessage = isHighLove ? getRandomMessage(loveMessages) : getRandomMessage(notSoHighLoveMessages);
  const response =
    `━━━━━━━⬣ *LOVE* ⬣━━━━━━━\n` +
    `*تم بنجاح💞, ${text} y @${m.sender.split('@')[0]} ${loveDescription}  ${lovePercentage}% من 100%*\n\n` +
    `*❥ Miku Bot*\n` +
    `━━━━━━━⬣ *LOVE* ⬣━━━━━━━`    
...
"《 ████████████》100%"
   ...
   let { key } = await conn.sendMessage(m.chat, {text: `*💞 ¡جاري تحميل نسبتك! 💞*`, mentions: conn.parseMention(response)}, {quoted: m})
